...
async def paper(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "start":
        ...
        stats = paper_trader.get_stats(user_id)
        positions = paper_trader.get_positions(user_id)
        msg = get_text(lang, "paper_status", capital=stats["capital"], equity=stats["equity"],
                       open_positions=stats["open_positions"], total_pnl=stats["total_pnl"])
        if positions:
            for p in positions:
                msg += f"\n{p['symbol']} @ {p['entry_price']:.2f} | SL: {p['sl']:.2f} | TP: {p['tp']:.2f} | PnL: {p['pnl_usdt']:.2f}$"
        else:
        ...
        paper_trader.open_position(user_id, symbol, price, sl, tp, qty)
        await respond(update, get_text(lang, "paper_opened", symbol=symbol, price=price, sl=sl, tp=tp))
