...
def get_history(symbol: str) -> pd.DataFrame:
    ...
    # Use FCS API for everything (Yahoo is blocked on Railway)
    if FCS_API_KEY:
        try:
            params = {
                "access_key": FCS_API_KEY,
                "symbol": realmarket_symbol(symbol_clean),
                "period": "1D"
            }
            cls = market_class(symbol_clean)
            if cls == "commodity":
                params["type"] = "commodity"
            elif cls == "forex":
                params["synthetic"] = "1"
...
def get_current_price(symbol: str) -> Tuple[Optional[float], str]:
    ...
    # Use FCS API for all prices
    if FCS_API_KEY:
        try:
            params = {"access_key": FCS_API_KEY, "symbol": realmarket_symbol(symbol_clean)}
            cls = market_class(symbol_clean)
            if cls == "commodity":
                params["type"] = "commodity"
            elif cls == "crypto":
            ...
            resp = requests.get("https://fcsapi.com/api-v4/forex/latest", params=params, timeout=15)
            if resp.status_code == 200:
                data = resp.json()
                if data.get("code") == 200:
            ...
            logger.warning(f"FCS price failed: {e}")
